...
public class UpperSnakeCaseTests
{
    private static readonly RecyclableMemoryStreamManager MemoryStreamManager = new();
    private static readonly IContentTypeProvider ContentTypeProvider = new FileExtensionContentTypeProvider();
    private static UpperSnakeCase UpperSnakeCaseReadWriter => new(MemoryStreamManager, ContentTypeProvider);
    ...
    public async ValueTask UpperSnakeCase_WhenGivenAValue_Should_WriteValidCamelCaseJsonObject(string strValue, int? intValue, bool boolValue, int [] arrayValue)
    {
        var obj = new
        {
            FirstValue = strValue,
        ...
        var content = await UpperSnakeCaseReadWriter.Write(obj);
    ...
    public async ValueTask UpperSnakeCase_WhenGivenAMixedContent_Should_ReadValidCamelCaseJsonObject(string strValue, int? intValue, bool boolValue, int [] arrayValue)
    {
        var json = 
            $@"{{
            ""FIRST_VALUE"":""{strValue}"",
        ...
        var result = await UpperSnakeCaseReadWriter.Read<SomeObject>(content);
    ...
    [InlineData("", int.MinValue, true, new[] { 21, 22, 23, 24 })]
    [InlineData("null", (int.MaxValue / 2), false, new[] { 25, 26, 27, 28 })]
    public async ValueTask UpperSnakeCase_WhenGivenAContentWithCamelCase_Should_ReadValidCamelCaseJsonObject(string strValue, int? intValue, bool boolValue, int [] arrayValue)
    {
        var json = 
            $@"{{
            ""firstValue"":""{strValue}"",
    ...
    [InlineData("", int.MinValue, true, new[] { 21, 22, 23, 24 })]
    [InlineData("null", (int.MaxValue / 2), false, new[] { 25, 26, 27, 28 })]
    public async ValueTask UpperSnakeCase_WhenGivenAContentWithLowerSnake_Should_ReadValidCamelCaseJsonObject(string strValue, int? intValue, bool boolValue, int [] arrayValue)
    {
        var json = 
            $@"{{
            ""first_value"":""{strValue}"",
