...
public class CamelCaseTests
{
    ...
    private static CamelCase CamelCaseReadWriter => new(MemoryStreamManager, ContentTypeProvider);
    ...
    public async ValueTask CamelCase_WhenGivenAValue_Should_WriteValidCamelCaseJsonObject(string strValue, int? intValue, bool boolValue, int [] arrayValue)
    {
        var obj = new
        {
            FirstValue = strValue,
        ...
        var content = await CamelCaseReadWriter.Write(obj);
    ...
    public async ValueTask CamelCase_WhenGivenAContent_Should_ReadValidCamelCaseJsonObject(string strValue, int? intValue, bool boolValue, int [] arrayValue)
    {
        var json = 
            $@"{{
            ""firstValue"":""{strValue}"",
        ...
        var result = await CamelCaseReadWriter.Read<SomeObject>(content);
    ...
    [InlineData("", int.MinValue, true, new[] { 21, 22, 23, 24 })]
    [InlineData("null", (int.MaxValue / 2), false, new[] { 25, 26, 27, 28 })]
    public async ValueTask CamelCase_WhenGivenAContentWithLowerSnakeCase_Should_ReadValidCamelCaseJsonObject(string strValue, int? intValue, bool boolValue, int [] arrayValue)
    {
        var json = 
            $@"{{
    ...
    [InlineData("", int.MinValue, true, new[] { 21, 22, 23, 24 })]
    [InlineData("null", (int.MaxValue / 2), false, new[] { 25, 26, 27, 28 })]
    public async ValueTask CamelCase_WhenGivenAContentWithUpperSnakeCase_Should_ReadValidCamelCaseJsonObject(string strValue, int? intValue, bool boolValue, int [] arrayValue)
    {
        var json = 
            $@"{{
            ""FIRST_VALUE"":""{strValue}"",
